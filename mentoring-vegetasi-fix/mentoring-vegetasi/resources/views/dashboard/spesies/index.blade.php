...
@section('modal')
    ...
    <x-form-modal title="Form Data spesies" id="formModal" action-route="{{ route('dashboard.spesies.store') }}">
        ...
        <div>
            <label for="fk_id_vegetasi" class="form-label">Kategori Vegetasi</label>
            <select name="fk_id_vegetasi" id="fk_id_vegetasi" class="form-select">
                <option selected disabled>Pilih kategori</option>
                @foreach($Vegetasis as $vegetasi)
                    <option value="{{ $vegetasi->id }}">{{ $vegetasi->nama_vegetasi }}</option>
                @endforeach
            </select>
        </div>
        <div>
    ...
    @foreach ($Spesies as $spesies)
        ...
        <x-form-modal title="Form Data Spesies" id="formModal-{{ $loop->iteration }}"
            ...
            <div>
                <label for="fk_id_vegetasi" class="form-label">Kategori Vegetasi</label>
                <select name="fk_id_vegetasi" id="fk_id_vegetasi" class="form-select" >
                    <option selected disabled>Jenis vegetasi</option>
                    @foreach($Vegetasis as $vegetasi)
                        <option value="{{ $vegetasi->id }}" {{ $vegetasi->id == $spesies->fk_id_vegetasi ? 'selected' : '' }}>
                            {{ $vegetasi->nama_vegetasi }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <x-form-modal title="Konfirmasi Penghapusan Data" id="deleteModal-{{ $loop->iteration }}"
            ...
            @method('DELETE')
            <h6><b>Apakah Anda ingin menghapus data spesies {{ $spesies->nama_spesies }}?</b></h6>
